...
send each to the capcheck/ai-image-detection API via HF Inference,
aggregate scores with temporal weighting (later frames weighted higher).

No fallbacks. Any failure propagates as an HTTP error.
"""
...
async def detect_video_deepfake(video_bytes: bytes, filename: str) -> dict:
    ...
    # Extract frames — raises ValueError if cv2 missing or video invalid
    frames = _extract_frames(video_bytes, num_frames=8)

    if not frames:
        raise ValueError(
            "Could not extract frames from video. "
            "Ensure OpenCV (cv2) is installed and the video file is valid."
        )

    # Analyse each frame concurrently (semaphore limits parallel HF calls)
    semaphore = asyncio.Semaphore(4)

    async def analyse_frame(pil_frame: Image.Image, idx: int) -> dict:
    ...
    frame_scores = await asyncio.gather(*[analyse_frame(f, i) for i, f in enumerate(frames)])
...
def _extract_frames(video_bytes: bytes, num_frames: int = 8) -> list[Image.Image]:
    """Extract evenly-spaced frames using OpenCV. Raises on failure."""
    import cv2
    import tempfile
    import os
